...
module.exports = async function handler(req, res) {
    ...
    if (type === 'realtime') {
        ...
        try {
            ...
            if (!data.price) throw new Error('Missing price field');
    ...
    if (type === 'history') {
        if (!TWELVE_DATA_API_KEY) return res.status(500).json({ error: 'Missing Twelve Data API key' });
        try {
            ...
            if (!response.ok) throw new Error(`Twelve Data status ${response.status}`);
            const data = await response.json();
            if (!data.values || !Array.isArray(data.values)) throw new Error('Missing values array');
            const history = data.values
                .map(item => ({ date: item.datetime.split(' ')[0], price: parseFloat(item.close) }))
